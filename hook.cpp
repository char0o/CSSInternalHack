...
void *WINAPI hookFn(char *hookedFn, char *hookFn, int copyBytesSize, unsigned char *backupBytes, std::string descr)
{
    ...
    {
        MessageBox(0, L"[hookFn] Failed to Backup Original Bytes for ", L":(", 0);
        return nullptr;
    }
    ...
    {
        MessageBox(0, L"[hookFn] Failed to set RXW for ", L":(", 0);
        return nullptr;
    }
    ...
    {
        MessageBox(0, L"[hookFn] Failed to Restore Protection for ", L"Ok", 0);
    }
...
BOOL WINAPI restore(char *fn, unsigned char *writeBytes, int writeSize, std::string descr)
{
    ...
    {
        MessageBox(0, L"[restore] Failed to set RWX for ", L":(", 0);
        return FALSE;
    }

    if (!WriteProcessMemory(GetCurrentProcess(), fn, writeBytes, writeSize, 0))
    {
        MessageBox(0, L"[restore] Failed to Write Memory for ", L":(", 0);
        return FALSE;
    }

    if (!VirtualProtect(fn, writeSize, oldProtect, &oldProtect))
    {
        MessageBox(0, L"[restore] Failed to  Restore Protection for ", L":(", 0);
        return FALSE;
    }
